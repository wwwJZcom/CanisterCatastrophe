...
public class FuelCanisterSpawner_Top : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // This timer has a very high chance of spawning a Fuel Canister, but only executes every 50 seconds.
        if (timePassed >= 50f)
        {
            randomChance();
        }
    ...
    private void SpawnFuel()
    {
        ...
        float rotationAngle = Random.Range(150.0f, 210.0f);
    ...
    private void randomChance()
    {
        ...
        if (randomValue >= 0.1f && randomValue <= 0.81f)
        {
            canSpawn = true;
            Debug.Log("TRUE!");
